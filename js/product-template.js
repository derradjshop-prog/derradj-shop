...
(function (root) {
  ...
  function buildProductView(p) {
    ...
    /* ── WhatsApp button — for subscriptions this is the ONLY ordering
       ...
       still send. Reuses the same site-wide WhatsApp number as the
       floating widget (whatsapp-float.js) — never a new one. ── */
    ...
    const waBtn = (isSubscription && !isAvail)
      ...
      : `<a href="https://wa.me/213542949967?text=${waMsg}" target="_blank" rel="noopener noreferrer" class="btn-whatsapp-sb">${waIcon} ${isSubscription ? 'اطلب عبر الواتساب' : 'الطلب عبر الواتساب'}</a>`;
